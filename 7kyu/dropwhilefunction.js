...
for those values. You want to remove the longest prefix of elements such that the predicate is
true for each element. We'll call this the dropWhile function. It accepts two arguments. 
...
*/
function dropWhile(seq, isEven){
    const index= seq.findIndex((item)=> !isEven(item));
    return seq=== -1? []:seq.slice(index);
}