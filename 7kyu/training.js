...
function isolateIt(arr){
  ...
  let receiver = [];
  for(let item of arr){
      let index = Math.floor(item.length/2);
      if(item.length % 2 == 0){
          receiver.push(item.slice(0,index)+"|"+item.slice(index))
      }else{
        receiver.push(item.slice(0,index)+"|"+item.slice(index+1));
    //   return item.slice(index,item.length);
      }
  }
  return receiver;
}
